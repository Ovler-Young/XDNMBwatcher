...
export async function botPO(ctx) {
  ...
  if (sub.length == 0) {
    ...
    if (find === -1) {
      ...
      // po is the last word
      await ctx.reply("正在获取po", { reply_to_message_id: ctx.update.message.message_id });
      ...
      if (po === undefined || po === id) {
        ...
        await ctx.reply("获取成功，为 " + po, { reply_to_message_id: ctx.update.message.message_id });
        try {
            ...
            await ctx.reply("添加失败" + e, { reply_to_message_id: ctx.update.message.message_id });
        }
      }
    }
  }
