        ...
        if (sub[index].ReplyCountAll === feed[i].reply_count || sub[index].active === false) {
          ...
          try {
            ...
            for (let j = 0; j < newreplycount; j++) {
              ...
              let rep = feed[i].recent_replies.toString().split("[")[1].split("]")[0].split(",").map((item) => parseInt(item));
              ...
              if (data.user_hash === sub[index].po || (sub[index].issingle === false && sub[index].writer.includes(data.user_hash))) {
                // if title is not empty or "无标题", we need to add it to the content
                if (data.title !== "" && data.title !== "无标题") {
                  content_all.push(`<a href="https://www.nmbxd1.com/t/${feed[i].id}?r=${data.id}">${data.title}</a><br/>`);
                } else {
                  content_all.push(`<a href="https://www.nmbxd1.com/t/${feed[i].id}?r=${data.id}">#${data.id}</a><br/>`);
                }
                // if so, we need to get the content
                content_all.push(data.content.replace(/<[^>]+>/g, "").replace(/&gt;&gt;No\.(\d+)/g, `<a href="https://www.nmbxd1.com/Home/Forum/ref?id=$1">>>No.$1</a>`)); //https://www.nmbxd1.com/Home/Forum/ref?id=57858642
                // if there is an image, we need to add it to the content
                if (data.ext !== "") {
                  content_all.push(`<img src="https://image.nmb.best/image/${data.img}${data.ext}">`);
                }
                // sub
                // unread
                unread += 1;
